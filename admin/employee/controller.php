	...
	function doInsert(){
		...
		if ( $_POST['FNAME'] == "" OR $_POST['LNAME'] == ""
			...
			redirect("index.php?view=add");
				...
				$sql = "SELECT * FROM tblapplicants WHERE APPLICANTID='" .$_POST['APPLICANTID']. "'";
				$mydb->setQuery($sql);
				...
				if ($maxrow > 0) { 
					...
					$emp->USERNAME			= trim($_POST['USERNAME']);
		 	...
		 	if ($age < 20 ){
		       ...
		       redirect("index.php?view=edit&id=".$_POST['APPLICANTID']);
		    	...
		    	@$datehired = date_format(date_create($_POST['EMP_HIREDDATE']),'Y-m-d');
					...
					$user = New User(); 
					$u_res = $user->single_user($_POST['APPLICANTID']);
